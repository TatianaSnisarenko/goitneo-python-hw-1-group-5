...
def get_birthdays_per_week(users):
    show_birthdays_per_week_from_date(users, datetime.today())
...
def get_birthdays_per_week_from_date(users, from_date):
    ...
    for user in users:
        ...
        if delta_days < 0:
            birthday_this_year = get_date(date.year+1, birthday.month, birthday.day)
    ...
    for key, value in result.items():
        formatted_list.append(f'{days_of_week[key.weekday()]}: {', '.join(value)}')
    return tuple(formatted_list)

def show_birthdays_per_week_from_date(users, from_date):
    for value in get_birthdays_per_week_from_date(users, from_date):
        print(value)
